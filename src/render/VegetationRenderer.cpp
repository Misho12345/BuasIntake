...
namespace game::render
{
    namespace
    {
        ...
        PlantVisualSpec plant_visual_spec(const vegetation::Plant& plant)
        {
            ...
            const float sprout_fraction =
                std::clamp((plant.age - vegetation::seed_to_sprout_time) / vegetation::sprout_growth_duration, 0.0f, 1.0f);
    ...
    {
        static constexpr std::array live_32_paths{"assets/images/vegetation/ground_plants.png",
                                                  "assets/images/vegetation/grass.png",
                                                  "assets/images/vegetation/flowers.png",
                                                  "assets/images/vegetation/bushes.png"};
        static constexpr std::array live_64_paths{"assets/images/vegetation/trees.png"};
        static constexpr std::array dead_32_paths{"assets/images/vegetation/ground_plants_dead.png",
        ...
        static constexpr std::array dead_64_paths{"assets/images/vegetation/trees_dead.png"};
    ...
    {
        for (auto& resources : batch_resources_)
            resources.destroy_graphics_resources();
    ...
    Result<void> VegetationRenderer::rebuild_instances(const terrain::PlanetTerrain& terrain,
                                                       ...
                                                       const resources::ResourceSystem& resources)
    {
        ...
        for (const auto index : vegetation.active_plant_indices())
        {
            ...
            const ivec2 coord{static_cast<int>(index % terrain.global_field_size().x),
            ...
            const SpriteInstance instance{.center_world = anchor_world,
                                          .up = up,
            ...
            {
                if (plant.family == vegetation::PlantFamily::Bush)
                    cached_woody_live32_instances_.push_back(instance);
                else cached_low_cover_live32_instances_.push_back(instance);
            }
            else
            {
                cached_instances_by_batch_[static_cast<std::size_t>(spec.batch_id)].push_back(instance);
            }
        }
        ...
        for (const auto& resource : resources.nodes())
        {
            ...
            const vec2 world_position =
                resource.surface_attached ? resource.anchor_world : terrain.global_sample_world_position(resource.coord);
            const std::size_t family_index = std::min<std::size_t>(resource.variant / 16u, dead_plant_sprite_families.size() - 1u);
            const auto batch_id =
                family_index == dead_plant_sprite_families.size() - 1u ? VegetationBatchId::Dead64 : VegetationBatchId::Dead32;
            const float texture_layer = static_cast<float>(family_index == dead_plant_sprite_families.size() - 1u ? 0u : family_index);

            const vec2 up = resource.surface_up.lengthSquared() > 1e-6f ? normalize(resource.surface_up * -1.0f)
    ...
    void VegetationRenderer::draw(const sf::View& view,
                                  ...
                                  const resources::ResourceSystem& resources)
    {
        ...
        if (last_vegetation_revision_ != vegetation.revision() || last_resource_revision_ != resources.nodes_revision())
        {
            if (const auto rebuild_result = rebuild_instances(terrain, vegetation, resources); !rebuild_result)
            {
        ...
        auto filter_visible_instances = [&](std::span<const SpriteInstance> source, std::vector<SpriteInstance>& visible_instances)
        {
            ...
            for (const auto& instance : source)
            {
                ...
                if (delta.x * delta.x + delta.y * delta.y > visible_radius_sq)
                    continue;
                visible_instances.push_back(instance);
            }
        };

        auto draw_batch = [&](const VegetationBatchId batch_id,
