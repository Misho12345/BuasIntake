...
namespace game::render
{
    ...
    class InstancedSpriteBatch final
    {
      public:
        ...
        {
            if (vao_.valid() && quad_vbo_.valid() && instance_vbo_.valid() && texture_array_.valid() && shader_.valid())
                return {};
            if (texture_paths.empty())
                return fail("InstancedSpriteBatch requires at least one texture path");
            ...
            for (const auto* path : texture_paths)
            {
                ...
                if (!image.loadFromFile(path))
                    return fail("Failed to load sprite image '{}'", path);
            ...
            static constexpr std::array quad_vertices{QuadVertex{.position = {-0.5f, 0.0f}, .uv = {0.0f, 1.0f}},
            ...
            glNamedBufferData(quad_vbo_.id(), static_cast<GLsizeiptr>(sizeof(quad_vertices)), quad_vertices.data(), GL_STATIC_DRAW);
            glNamedBufferData(instance_vbo_.id(), static_cast<GLsizeiptr>(sizeof(SpriteInstance)), nullptr, GL_DYNAMIC_DRAW);
            ...
            glTextureStorage3D(texture_array_.id(),
                               1,
                               GL_RGBA8,
            ...
            {
                glTextureSubImage3D(texture_array_.id(),
                                    0,
                                    0,
                                    0,
        ...
        void upload_instances(std::span<const SpriteInstance> instances)
        {
            ...
            {
                glNamedBufferData(instance_vbo_.id(),
            ...
            else
            {
                glNamedBufferSubData(
                    instance_vbo_.id(), 0, static_cast<GLsizeiptr>(instances.size() * sizeof(SpriteInstance)), instances.data());
            }
        ...
        {
            if (instance_count_ == 0 || !shader_.valid() || !vao_.valid() || !texture_array_.valid())
                return;
        ...
        {
            return vao_.valid() && quad_vbo_.valid() && instance_vbo_.valid() && texture_array_.valid() && shader_.valid();
        }
