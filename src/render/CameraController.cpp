...
{
    void CameraController::set_world_span(const vec2 world_span) { settings_.world_span = world_span; }

    void CameraController::reset_follow() { follow_initialized_ = false; }

    void CameraController::zoom_by_scroll(const float scroll_delta)
    {
    ...
    void CameraController::sync_to_player(const world::World& world, const float dt, const bool move_input_active)
    {
        ...
        const float position_alpha = smooth_factor(
            move_input_active ? settings_.follow_smoothing : settings_.recenter_smoothing, dt);
        focus_world_ = lerp(focus_world_, desired_focus, position_alpha);

        const vec2 planet_center                    = world.terrain().planet_center();
        const bool is_repositioning = !move_input_active || player_delta.lengthSquared() > follow_threshold_sq;
