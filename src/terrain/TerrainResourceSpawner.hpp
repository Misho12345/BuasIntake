...
namespace game::terrain
{
    ...
    {
    public:
        // this whole pass is basically where the dead procedural field turns into actual things to look at and collect
        // the rules are intentionally messy because straight random placement looked fake and ignored cave shape wetness and spacing in all the wrong ways
            ...
            AddResourceNode&&                                    add_resource_node)
        {
            ...
            {
                const float value = std::sin(x * 12.9898f + y * 78.233f + static_cast<float>(hash_seed) * 0.013f) *
                        43758.5453f;
            ...
            auto choose_variant_row = [hash01, seed](const ivec2 coord, const std::uint32_t seed_offset)
            {
                return static_cast<std::uint8_t>(std::clamp(
                    static_cast<int>(hash01(static_cast<float>(coord.x), static_cast<float>(coord.y),
                                            seed + seed_offset) * 16.0f), 0, 15));
            };
            ...
            for (int y = 1; y < static_cast<int>(global_field_size.y) - 1; ++y)
            {
                for (int x = 1; x < static_cast<int>(global_field_size.x) - 1; ++x)
                {
                    ...
                    if (!cave && has_occupied_neighbor(coord, ground_resource_spacing_radius)) continue;

                    emit_node(
                        kind, coord,
                        choose_variant_row(coord, 19u),
                        cave,
                        true,
            ...
            for (int y = 1; y < static_cast<int>(global_field_size.y) - 1; ++y)
            {
                for (int x = 1; x < static_cast<int>(global_field_size.x) - 1; ++x)
                {
                    ...
                    if (occupied_samples.contains(sample_key(coord)) ||
                        has_occupied_neighbor(coord, ground_resource_spacing_radius))
                        continue;
                    ...
                    const float seam_noise =
                            std::abs(perlin_noise(world * 0.182f + vec2{ -7.0f, 19.0f }, seed + 4513u));
            ...
            for (int y = 1; y < static_cast<int>(global_field_size.y) - 1; ++y)
            {
                for (int x = 1; x < static_cast<int>(global_field_size.x) - 1; ++x)
                {
                    ...
                    const auto family_slot = std::min<std::size_t>(
                        family_roll * static_cast<float>(family_pool.size()),
                        family_pool.size() - 1u);
                    ...
                    const std::uint8_t variant = static_cast<std::uint8_t>(
                        family_index * 16u + choose_variant_row(coord, 4673u));
