...
namespace game::terrain
{
	namespace
	{
		...
		bool is_convex_polygon(const std::span<const vec2> points)
		{
			...
			for (std::size_t i = 0; i < points.size(); ++i)
			{
				...
				if (cross(a, b, c) < -1e-4f) return false;
			}
		...
		{
			if (source.size() < 3u || source.size() > B2_MAX_POLYGON_VERTICES) return false;
			if (std::abs(signed_area(source)) <= minimum_polygon_area) return false;
			...
			if (!b2ValidateHull(&hull)) return false;
		...
		{
			if (loop.size() < 3u) return 0u;

			if (signed_area(loop) < 0.0f) std::ranges::reverse(loop);
			if (std::abs(signed_area(loop)) <= minimum_polygon_area) return 0u;
			...
			while (indices.size() > 3u)
			{
				...
				for (std::size_t i = 0; i < indices.size(); ++i)
				{
					...
					if (cross(a, b, c) <= 1e-4f) continue;

					bool contains_point = false;
					for (const auto candidate_index : indices)
					{
						if (candidate_index == previous_index || candidate_index == current_index || candidate_index == next_index) continue;
						if (!point_in_triangle(loop[candidate_index], a, b, c)) continue;
					...
					if (create_sensor_polygon(body, shape_def, triangle)) ++created_count;
					indices.erase(indices.begin() + static_cast<std::ptrdiff_t>(i));
					clipped_ear = true;
					break;
				}

				if (!clipped_ear) break;
			}
	...
	TerrainCollider::~TerrainCollider()
	{
		...
		if (b2Body_IsValid(water_sensor_body_)) b2DestroyBody(water_sensor_body_);
	}
		...
		const std::vector<std::vector<vec2>>& water_loops)
	{
		...
		if (loops.empty() && paths.empty() && water_loops.empty()) return;
		...
		if (!loops.empty() || !paths.empty())
		{
			...
			for (const auto& loop : loops)
			{
				...
				b2CreateSegmentShape(terrain_body_, &segment_shape_def, &closing_segment);
				++created_count;
			}
	...
	{
		if (!b2Body_IsValid(water_sensor_body_)) return;

		const auto currently_enabled = b2Body_IsEnabled(water_sensor_body_);
		if (enabled && !currently_enabled) b2Body_Enable(water_sensor_body_);
		if (!enabled && currently_enabled) b2Body_Disable(water_sensor_body_);
	}
