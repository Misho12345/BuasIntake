...
namespace game::terrain
{
    ...
    {
        return {settings.world_center.x + (settings.chunk_coord.x - 0.5f * settings.chunk_grid_size.x) * settings.chunk_size.x,
                settings.world_center.y + (settings.chunk_coord.y - 0.5f * settings.chunk_grid_size.y) * settings.chunk_size.y};
    }
    ...
    {
        return {settings.chunk_size.x / std::max(settings.field_size.x - 1.0f, 1.0f),
                settings.chunk_size.y / std::max(settings.field_size.y - 1.0f, 1.0f)};
    }

    inline uvec2 padded_field_size(const ChunkSettings& settings)
    {
        return {settings.field_size.x + settings.field_padding.x * 2u, settings.field_size.y + settings.field_padding.y * 2u};
    }

    inline ivec2 chunk_sample_stride(const ChunkSettings& settings)
    {
        return {static_cast<std::int32_t>(std::max(settings.field_size.x, 1u) - 1u),
                static_cast<std::int32_t>(std::max(settings.field_size.y, 1u) - 1u)};
    }

    inline ivec2 field_padding(const ChunkSettings& settings)
    {
        return {static_cast<std::int32_t>(settings.field_padding.x), static_cast<std::int32_t>(settings.field_padding.y)};
    }
    ...
    inline vec2 field_origin(const ChunkSettings& settings)
    {
        ...
        const auto min = chunk_min(settings);
        return {min.x - terrain_cell_size.x * static_cast<float>(settings.field_padding.x),
