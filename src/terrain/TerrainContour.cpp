...
namespace game::terrain
{
    namespace
    {
        ...
        float distance_to_segment(const vec2& point, const vec2& a, const vec2& b)
        {
            ...
            if (ab_length_sq <= std::numeric_limits<float>::epsilon())
        ...
        bool touches_chunk_boundary(const std::span<const vec2> points, const ChunkSettings& settings, const float epsilon)
        {
            ...
            const vec2 padded_min{min.x - terrain_cell_size.x * static_cast<float>(settings.field_padding.x),
                                  min.y - terrain_cell_size.y * static_cast<float>(settings.field_padding.y)};
            const vec2 padded_max{max.x + terrain_cell_size.x * static_cast<float>(settings.field_padding.x),
                                  max.y + terrain_cell_size.y * static_cast<float>(settings.field_padding.y)};

            for (const auto& point : points)
            {
                if (std::abs(point.x - min.x) <= epsilon || std::abs(point.x - max.x) <= epsilon || std::abs(point.y - min.y) <= epsilon ||
                    std::abs(point.y - max.y) <= epsilon || std::abs(point.x - padded_min.x) <= epsilon ||
                    std::abs(point.x - padded_max.x) <= epsilon || std::abs(point.y - padded_min.y) <= epsilon ||
                    std::abs(point.y - padded_max.y) <= epsilon)
                {
                    return true;
                }
            }
    ...
    TerrainContour::ExtractedContours TerrainContour::extract_contours(const std::vector<vec2>& boundary_vertices,
                                                                       const std::vector<TerrainGenerator::BoundaryEdge>& edges)
    {
        ...
        {
            if (edges[i].a >= boundary_vertices.size() || edges[i].b >= boundary_vertices.size())
                continue;
        ...
        auto consume_path = [&](const std::size_t start_edge_index)
        {
            ...
            while (true)
            {
                ...
                if (next == outgoing_edges.end())
                    break;

                const auto next_edge =
                    std::ranges::find_if(next->second, [&visited](const std::size_t candidate_edge) { return !visited[candidate_edge]; });

                if (next_edge == next->second.end())
                    break;
            ...
            if (closed)
            {
                if (!contour.empty())
                    contour.pop_back();
                if (contour.size() >= 3)
                    contours.loops.push_back(std::move(contour));
            }
        ...
        for (std::size_t i = 0; i < edges.size(); ++i)
        {
            ...
            if (const auto start_vertex = edges[i].a; incoming_count[start_vertex] != 1u || outgoing_count[start_vertex] != 1u)
                consume_path(i);
        }
    ...
    std::vector<vec2> TerrainContour::simplify_contour(const std::vector<vec2>& contour,
                                                       const bool closed,
                                                       const float min_segment_length,
                                                       const float collinear_epsilon)
    {
        ...
        while (changed && simplified.size() >= min_points)
        {
            ...
            for (std::size_t i = 0; i < simplified.size(); ++i)
            {
                ...
                const bool has_short_edge = first_length_sq <= min_segment_length_sq || second_length_sq <= min_segment_length_sq;

                float alignment = 1.0f;
                if (first_length_sq > std::numeric_limits<float>::epsilon() && second_length_sq > std::numeric_limits<float>::epsilon())
                {
                    alignment = first_edge.dot(second_edge) / std::sqrt(first_length_sq * second_length_sq);
                }

                const float corner_distance = distance_to_segment(current, prev, following);
                ...
                if (const bool nearly_collinear =
                        alignment >= combine_dot_threshold && corner_distance <= collinear_epsilon;
            ...
            if (next.size() < min_points || next.size() == simplified.size())
                break;
    ...
    TerrainContour::ScoredResult TerrainContour::score_and_filter(TerrainGenerator::RawPipelineResult raw, const ChunkSettings& settings)
    {
        ...
        if (raw.boundary_vertices.empty() || raw.boundary_edges.empty())
            return result;
        ...
        for (const auto& path : open_paths)
        {
            ...
            if (length < min_path_length && !touches_chunk_boundary(simplified, settings, boundary_epsilon))
                continue;
