...
namespace game::terrain
{
    namespace
    {
            ...
            const float                 epsilon)
        {
            ...
            {
                if (std::abs(point.x - min.x) <= epsilon ||
                    std::abs(point.x - max.x) <= epsilon ||
                    std::abs(point.y - min.y) <= epsilon ||
                    std::abs(point.y - max.y) <= epsilon ||
                    std::abs(point.x - padded_min.x) <= epsilon ||
                    std::abs(point.x - padded_max.x) <= epsilon ||
                    std::abs(point.y - padded_min.y) <= epsilon ||
    ...
    TerrainContour::ExtractedContours TerrainContour::extract_contours(
        ...
        auto consume_path = [&](const std::size_t start_edge_index)
        {
            ...
            while (true)
            {
                ...
                const auto next_edge =
                        std::ranges::find_if(
                            next->second,
                            [&visited](const std::size_t candidate_edge) { return !visited[candidate_edge]; });
            ...
            else if (contour.size() >= 2) contours.open_paths.push_back(std::move(contour));
        };
    ...
    std::vector<vec2> TerrainContour::simplify_contour
     ...
     const float              collinear_epsilon)
    {
        ...
        while (changed && simplified.size() >= min_points)
        {
            ...
            for (std::size_t i = 0; i < simplified.size(); ++i)
            {
                ...
                const bool  has_short_edge   =
                        first_length_sq <= min_segment_length_sq ||
                        second_length_sq <= min_segment_length_sq;

                float alignment = 1.0f;

                if (first_length_sq > eps && second_length_sq > eps)
                {
                    alignment =
                        first_edge.dot(second_edge) /
                        std::sqrt(first_length_sq * second_length_sq);
                }
            ...
            if (next.size() < min_points ||
                next.size() == simplified.size())
                break;
    ...
    TerrainContour::ScoredResult TerrainContour::score_and_filter(
