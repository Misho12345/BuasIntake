...
namespace game::terrain
{
    ...
    {
        inline TerrainSampleBounds clamp_sample_bounds(const ivec2 min_coord, const ivec2 max_coord, const uvec2 field_size)
        {
            return {.min = {std::clamp(min_coord.x, 0, static_cast<int>(field_size.x) - 1),
                            std::clamp(min_coord.y, 0, static_cast<int>(field_size.y) - 1)},
                    .max = {std::clamp(max_coord.x, 0, static_cast<int>(field_size.x) - 1),
                            std::clamp(max_coord.y, 0, static_cast<int>(field_size.y) - 1)}};
        }

        inline TerrainSampleBounds expand_sample_bounds(const TerrainSampleBounds& bounds, const int radius_cells, const uvec2 field_size)
        {
            return clamp_sample_bounds({bounds.min.x - radius_cells, bounds.min.y - radius_cells},
        ...
        {
            return {.min = {std::min(lhs.min.x, rhs.min.x), std::min(lhs.min.y, rhs.min.y)},
        ...
        inline TerrainSampleBounds intersect_sample_bounds(const TerrainSampleBounds& lhs, const TerrainSampleBounds& rhs)
        {
            return {.min = {std::max(lhs.min.x, rhs.min.x), std::max(lhs.min.y, rhs.min.y)},
                    .max = {std::min(lhs.max.x, rhs.max.x), std::min(lhs.max.y, rhs.max.y)}};
        }

        template <typename CollectWaterComponent>
        std::vector<TerrainWetnessComponent> collect_wetness_components(const std::span<const TerrainGenerator::FieldSample> global_field,
                                                                        ...
                                                                        CollectWaterComponent&& collect_water_component)
        {
            ...
            auto has_water = [](const TerrainGenerator::FieldSample& sample) { return std::min(-sample.terrain, sample.water) > 1e-4f; };
            ...
            auto component_wetness_distance = [min_cell_extent, max_wetness_radius_cells](const std::size_t water_sample_count)
            {
                ...
                const float radius_cells = std::clamp(base_wetness_radius_cells + equivalent_radius_cells * pond_radius_scale,
                                                      1.0f,
            ...
            for (int y = discovery_bounds.min.y; y <= discovery_bounds.max.y; ++y)
            {
                for (int x = discovery_bounds.min.x; x <= discovery_bounds.max.x; ++x)
                {
                    ...
                    if (!has_water(global_field[field_index(coord)]))
                        continue;

                    const auto key = sample_key(coord);
                    if (!visited_water.insert(key).second)
                        continue;
                    ...
                    const int radius_cells = std::max(1, static_cast<int>(std::ceil(max_distance / std::max(min_cell_extent, 1e-6f))));
                    const auto component_bounds = expand_sample_bounds(water_bounds, radius_cells, global_field_size);
                    if (!sample_bounds_intersect(component_bounds, discovery_bounds))
                        continue;

                    components.push_back({.water_cells = std::move(water_cells),
        ...
        void recompute_around(std::span<TerrainGenerator::FieldSample> global_field,
                              const uvec2 global_field_size,
                              ...
                              CollectWaterComponent&& collect_water_component)
        {
            if (changed_coords.empty() || global_field.empty())
                return;
            ...
            {
                if (max_distance <= 1e-6f || distance >= max_distance)
                    return 0.0f;

                const float saturated_band = max_distance * 0.25f;
                if (distance <= saturated_band)
                    return 1.0f;

                const float normalized =
                    std::clamp(1.0f - (distance - saturated_band) / std::max(max_distance - saturated_band, 1e-6f), 0.0f, 1.0f);
                return normalized * normalized * (3.0f - 2.0f * normalized);
            };

            auto bounds_contains = [](const TerrainSampleBounds& bounds, const ivec2 coord)
            { return coord.x >= bounds.min.x && coord.y >= bounds.min.y && coord.x <= bounds.max.x && coord.y <= bounds.max.y; };
            ...
            if (!has_water_in_discovery)
            {
                ...
                for (int y = discovery_bounds.min.y; y <= discovery_bounds.max.y; ++y)
                {
                    for (int x = discovery_bounds.min.x; x <= discovery_bounds.max.x; ++x)
                    {
                        ...
                        if (std::abs(sample.wetness - next_wetness) <= 1e-6f)
                            continue;
            ...
            std::vector<float> best_wetness(static_cast<std::size_t>(affected_width) * static_cast<std::size_t>(affected_height), 0.0f);

            auto affected_index = [affected_bounds, affected_width](const ivec2 coord)
            {
                return static_cast<std::size_t>(coord.y - affected_bounds.min.y) * static_cast<std::size_t>(affected_width) +
            ...
            for (const auto& component : components)
            {
                ...
                {
                    return static_cast<std::size_t>(coord.y - propagation_bounds.min.y) * static_cast<std::size_t>(propagation_width) +
                ...
                auto try_push = [&](const ivec2 coord, const float distance)
                {
                    if (!bounds_contains(propagation_bounds, coord) || !bounds_contains(affected_bounds, coord) ||
                        ...
                        return;
                    if (!is_solid(global_field[field_index(coord)]))
                        return;

                    auto& best_distance = best_distances[propagation_index(coord)];
                    if (distance + 1e-5f >= best_distance || distance > component.max_distance)
                        return;
                ...
                while (!frontier.empty())
                {
                    ...
                    if (distance > best_distances[propagation_index(coord)] + 1e-5f)
                        continue;

                    best_wetness[affected_index(coord)] =
                        std::max(best_wetness[affected_index(coord)], wetness_strength(distance, component.max_distance));

                    for (const auto& offset : wetness_neighbors)
                    {
                        try_push({coord.x + offset.x, coord.y + offset.y}, distance + neighbor_distance(offset));
                    }
                }
            }
