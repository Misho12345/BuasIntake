...
namespace game::terrain
{
    namespace terrain_greenness
    {
        ...
        void recompute(std::span<TerrainGenerator::FieldSample> global_field,
        ...
        {
            if (global_field.empty() || plant_samples.empty())
                return;
            ...
            std::vector<float> next_greenness(global_field.size(), 0.0f);
            for (const auto index : active_plant_indices)
            {
                if (index >= plant_samples.size())
                    continue;
                const auto& plant = plant_samples[index];
                if (plant.stage == vegetation::PlantStage::Empty || plant.stage == vegetation::PlantStage::Seeded)
                    continue;

                const ivec2 coord{static_cast<int>(index % global_field_size.x), static_cast<int>(index / global_field_size.x)};
                const auto& sample = global_field[index];
                if (!is_solid(sample))
                    continue;

                const float wetness_factor = std::clamp((sample.wetness - 0.10f) / 0.30f, 0.0f, 1.0f);
                if (wetness_factor <= 1e-4f)
                    continue;
                ...
                const int radius_x = std::max(1, static_cast<int>(std::ceil(radius_world / std::max(terrain_cell_size.x, 1e-4f))));
                const int radius_y = std::max(1, static_cast<int>(std::ceil(radius_world / std::max(terrain_cell_size.y, 1e-4f))));
                const vec2 center = plant.anchor_world.lengthSquared() > 1e-6f ? plant.anchor_world : global_sample_world_position(coord);

                for (int y = std::max(0, coord.y - radius_y); y <= std::min(static_cast<int>(global_field_size.y) - 1, coord.y + radius_y);
                     ++y)
                {
                    for (int x = std::max(0, coord.x - radius_x);
                         x <= std::min(static_cast<int>(global_field_size.x) - 1, coord.x + radius_x);
                         ++x)
                    {
                        const ivec2 target_coord{x, y};
                        const auto target_index = global_field_index(target_coord);
                        if (!is_solid(global_field[target_index]))
                            continue;
                        ...
                        float falloff = 0.0f;
                        if (distance <= radius_world * 0.72f)
                            falloff = 1.0f;
                        else
                        {
            ...
            for (int y = 0; y < static_cast<int>(global_field_size.y); ++y)
            {
                for (int x = 0; x < static_cast<int>(global_field_size.x); ++x)
                {
                    ...
                    if (std::abs(sample.greenness - greenness) <= 1e-6f)
                        continue;
