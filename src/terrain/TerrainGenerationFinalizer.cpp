...
namespace game::terrain
{
        ...
        const TerrainGenerationCallbacks& callbacks)
    {
        ...
        if (view.global_field.empty()) return changed_coords;
        ...
        {
            std::vector smoothed_terrain(view.global_field.size(), 0.0f);
            std::vector smoothed_water(view.global_field.size(), 0.0f);
            ...
            for (int y = 1; y < static_cast<int>(view.global_field_size.y) - 1; ++y)
            {
                for (int x = 1; x < static_cast<int>(view.global_field_size.x) - 1; ++x)
                {
                    ...
                    smoothed_terrain[index] =
                            std::lerp(view.global_field[index].terrain, terrain_total / std::max(terrain_weight, 1e-4f),
                                      0.42f);
                    if (water_weight > 0.0f)
                    {
                        smoothed_water[index] = std::lerp(view.global_field[index].water, water_total / water_weight,
                                                          0.55f);
                    }
                }
            }
    ...
    void terrain_generation_finalizer::generate_resource_nodes(
        const TerrainGenerationFieldView& view,
        const TerrainGenerationCallbacks& callbacks)
    {
        TerrainResourceSpawner::generate(
            view.global_field,
