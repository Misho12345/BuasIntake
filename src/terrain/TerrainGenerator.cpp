...
namespace game::terrain
{
    namespace
    {
        ...
        {
            return (settings.field_size.x - 1u + settings.field_padding.x) * (settings.field_size.y - 1u + settings.field_padding.y);
        }
    ...
    Result<void> TerrainGenerator::initialize(const ChunkSettings& settings)
    {
        ...
        auto load_compute_shader = [](const fs::path& path, gfx::Shader& shader) -> Result<void>
        {
            ...
            if (!loaded_shader)
                return fail(loaded_shader.error());
    ...
    TerrainGenerator::TerrainGenerator(TerrainGenerator&& other) noexcept
        : settings_{other.settings_}, terrain_shader_{std::move(other.terrain_shader_)}, cave_shader_{std::move(other.cave_shader_)},
          pond_shader_{std::move(other.pond_shader_)}, water_smooth_shader_{std::move(other.water_smooth_shader_)},
          edge_shader_{std::move(other.edge_shader_)}, mesh_shader_{std::move(other.mesh_shader_)},
          field_texture_{std::move(other.field_texture_)}, scratch_field_texture_{std::move(other.scratch_field_texture_)},
          ...
          mesh_vertices_buffer_{std::move(other.mesh_vertices_buffer_)}, mesh_indices_buffer_{std::move(other.mesh_indices_buffer_)},
          boundary_edges_buffer_{std::move(other.boundary_edges_buffer_)}, counters_buffer_{std::move(other.counters_buffer_)},
    ...
    Result<void> TerrainGenerator::dispatch()
    {
        ...
        const std::array generation_passes{
            gfx::ComputeDispatcher::Pass{.shader = &terrain_shader_,
                                         .groups = groups,
            ...
            gfx::ComputeDispatcher::Pass{.shader = &cave_shader_,
                                         .groups = groups,
                                         .configure = [this, layout](const gfx::Shader& shader) { bind_cave_pass(shader, layout); },
                                         .barrier_after = GL_SHADER_IMAGE_ACCESS_BARRIER_BIT},
            gfx::ComputeDispatcher::Pass{.shader = &pond_shader_,
                                         .groups = groups,
    ...
    Result<void> TerrainGenerator::smooth_water_field(const std::uint32_t iterations)
    {
        ...
        for (std::uint32_t iteration = 0; iteration < iterations; ++iteration)
        {
            ...
            const std::array smooth_passes{
                gfx::ComputeDispatcher::Pass{.shader = &water_smooth_shader_,
                                             .groups = groups,
            ...
            if (auto res = gfx::ComputeDispatcher::run(smooth_passes); !res)
        ...
        {
            const auto size = field_texture_.size();
            glCopyImageSubData(scratch_field_texture_.native_handle(),
                               GL_TEXTURE_2D,
                               0,
                               0,
                               0,
    ...
    Result<void> TerrainGenerator::dispatch_surface_rebuild(const std::uint32_t channel_index, const float iso)
    {
        ...
        const std::array rebuild_passes{
            gfx::ComputeDispatcher::Pass{.shader = &edge_shader_,
                                         .groups = groups,
            ...
            gfx::ComputeDispatcher::Pass{.shader = &mesh_shader_,
                                         .groups = groups,
                                         .configure = [this, layout, channel_index, iso](const gfx::Shader& shader)
    ...
    Result<void> TerrainGenerator::upload_field(const std::span<const FieldSample> field_samples)
    {
        ...
        {
            return fail("Uploaded field data size {} does not match texture extent sample count {}", field_samples.size(), expected_count);
        }

        glTextureSubImage2D(field_texture_.native_handle(),
                            0,
                            0,
                            0,
                            static_cast<GLsizei>(size.x),
    ...
    Result<std::vector<TerrainGenerator::FieldSample>> TerrainGenerator::read_field() const
    {
        ...
        glGetTextureImage(field_texture_.native_handle(),
                          0,
                          GL_RGBA,
                          GL_FLOAT,
    ...
    void TerrainGenerator::bind_water_smooth_pass(const gfx::Shader& shader,
    ...
    void TerrainGenerator::bind_surface_edge_pass(const gfx::Shader& shader,
    ...
    void TerrainGenerator::bind_surface_mesh_pass(const gfx::Shader& shader,
                                                  const FieldLayout& layout,
                                                  const std::uint32_t channel_index,
                                                  const float iso) const
    {
        ...
        shader.set_uniform("uFieldSize",
                           ivec2{static_cast<std::int32_t>(settings_.field_size.x), static_cast<std::int32_t>(settings_.field_size.y)});
    ...
    Result<TerrainGenerator::RawPipelineResult> TerrainGenerator::consume_readback()
    {
        ...
        {
            return fail("TerrainGenerator surface rebuild exceeded GPU buffer capacity (boundary vertices: {}/{}, mesh vertices: {}/{}, "
        ...
        if (boundary_vertex_count > boundary_vertex_capacity || vertex_count > mesh_vertex_capacity || index_count > mesh_index_capacity ||
            edge_count > boundary_edge_capacity)
        {
            return fail("TerrainGenerator surface rebuild reported invalid GPU counts (boundary vertices: {}/{}, mesh vertices: {}/{}, "
                        "mesh indices: {}/{}, boundary edges: {}/{})",
                        boundary_vertex_count,
                        boundary_vertex_capacity,
                        vertex_count,
