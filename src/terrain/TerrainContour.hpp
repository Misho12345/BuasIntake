...
namespace game::terrain
{
    class TerrainContour final
    {
      public:
        ...
        static ExtractedContours extract_contours(const std::vector<vec2>& boundary_vertices,
                                                  const std::vector<TerrainGenerator::BoundaryEdge>& edges);
        static std::vector<vec2> simplify_contour(const std::vector<vec2>& contour,
                                                  bool closed,
                                                  float min_segment_length,
                                                  float collinear_epsilon);
        static ScoredResult score_and_filter(TerrainGenerator::RawPipelineResult raw, const ChunkSettings& settings);
    };
}
