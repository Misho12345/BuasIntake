...
namespace game::terrain
{
    namespace
    {
        ...
        {
            if (values.empty() || size.x < 2u || size.y < 2u)
                return {};
    ...
    void TerrainWaterColliderBuilder::rebuild(TerrainColliderManager& collider_manager, const water::GridView& grid)
    {
        ...
        for (std::uint32_t y = 0; y < grid.field_size.y; ++y)
        {
            for (std::uint32_t x = 0; x < grid.field_size.x; ++x)
            {
                ...
                if (!water::has_water(grid.field_samples[start_index]))
                    continue;
                ...
                const uvec2 local_size{static_cast<std::uint32_t>(bounds.max.x - bounds.min.x + 1),
                ...
                for (std::uint32_t local_y = 0; local_y < local_size.y; ++local_y)
                {
                    ...
                    {
                        const ivec2 global_coord{bounds.min.x + static_cast<int>(local_x), bounds.min.y + static_cast<int>(local_y)};
                        const auto index = static_cast<std::size_t>(local_y) * local_size.x + local_x;
                        if (!component_keys.contains(sample_key(global_coord)))
                            continue;
                        local_values[index] =
                            water::combined_water_field(grid.field_samples[water::global_field_index(grid, global_coord)]);
                    }
                }

                auto loops =
                    extract_contour_loops_from_scalar_field(local_values,
                                                            local_size,
