...
namespace game::terrain
{
    class TerrainBrushSystem final
    {
      public:
        ...
        static Result apply_edit(std::span<TerrainGenerator::FieldSample> global_field,
                                 ...
                                 ClearSolidSample&& clear_solid_sample)
        {
            ...
            if (radius <= 0.0f || unit_budget == 0u)
                return {};

            const vec2 edit_center{edit.position_radius_strength.x, edit.position_radius_strength.y};
            if (!circle_overlaps_rect(edit_center, radius, grid_min, grid_max))
                return {};
            ...
            collect_candidates<Candidate>(global_field,
                                          global_field_size,
                                          global_field_origin,
                                          terrain_cell_size,
                                          settings,
            ...
            std::ranges::sort(candidates,
                              ...
                              {
                                  if (std::abs(lhs.falloff - rhs.falloff) > 1e-6f)
                                      return lhs.falloff > rhs.falloff;
                                  return lhs.distance_to_center < rhs.distance_to_center;
                              });

            result.candidates =
                static_cast<std::uint32_t>(std::min<std::size_t>(candidates.size(), std::numeric_limits<std::uint32_t>::max()));

            apply_candidates<Candidate>(global_field,
                                        settings,
                                        candidates,
                                        signed_strength,
                                        unit_budget,
      ...
      private:
        static bool circle_overlaps_rect(const vec2 center, const float radius, const vec2 rect_min, const vec2 rect_max)
        {
        ...
        static bool has_water(const TerrainGenerator::FieldSample& sample)
        {
            return water::has_water(sample);
        }

        static bool is_solid(const TerrainGenerator::FieldSample& sample)
        {
        ...
        static bool point_inside_brush_blocker(const vec2 point, const GroundBrushBlocker& blocker, const vec2 padding)
        {
            ...
            return std::abs(local_x) <= blocker.half_extents.x + padding.x && std::abs(local_y) <= blocker.half_extents.y + padding.y;
        }
        ...
        {
            const auto min_x = static_cast<int>(std::floor((edit_center.x - radius - global_field_origin.x) / terrain_cell_size.x));
            const auto min_y = static_cast<int>(std::floor((edit_center.y - radius - global_field_origin.y) / terrain_cell_size.y));
            const auto max_x = static_cast<int>(std::ceil((edit_center.x + radius - global_field_origin.x) / terrain_cell_size.x));
            const auto max_y = static_cast<int>(std::ceil((edit_center.y + radius - global_field_origin.y) / terrain_cell_size.y));
            ...
            for (int y = clamped_min_y; y <= clamped_max_y; ++y)
            {
                for (int x = clamped_min_x; x <= clamped_max_x; ++x)
                {
                    ...
                    if (blocker.has_value() &&
                        point_inside_brush_blocker(world, *blocker, {terrain_cell_size.x * 0.35f, terrain_cell_size.y * 0.35f}))
                    {
                        continue;
                    }

                    if (digging && is_dig_protected(coord))
                        continue;
                    ...
                    const float next_terrain = clamp_terrain_density(
                        hard_dig ? std::min(sample.terrain, signed_strength) : sample.terrain + signed_strength * falloff, world, settings);
                    float next_water = sample.water;
                    if (next_terrain >= 0.0f || !had_water)
                    {
                        next_water = dry_water_density(TerrainGenerator::FieldSample{
                            .terrain = next_terrain, .water = sample.water, .wetness = sample.wetness, .greenness = sample.greenness});
                    }

                    const bool local_changed =
                        std::abs(next_terrain - sample.terrain) > 1e-6f || std::abs(next_water - sample.water) > 1e-6f;
                    if (!local_changed)
                        continue;
                                     ...
                                     Result& result)
        {
            for (std::size_t i = 0; i < candidates.size(); ++i)
            {
                ...
                const float next_terrain = clamp_terrain_density(hard_dig ? std::min(sample.terrain, signed_strength)
                                                                          : sample.terrain + signed_strength * candidates[i].falloff,
                                                                 world,
                                                                 settings);
