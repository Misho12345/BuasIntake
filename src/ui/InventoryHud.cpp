...
namespace game::ui
{
    namespace
    {
        ...
        sf::Text make_counter_text(const sf::Font& font, const std::uint32_t value, const unsigned int character_size = 18u)
        {
    ...
    void InventoryHud::draw(sf::RenderTarget& target, const resources::HudState& hud_state) const
    {
        ...
        const std::array<CounterEntry, resources::hud_counter_count> entries{{
        ...
        for (std::size_t i = 0; i < entries.size(); ++i)
        {
            ...
            const auto& counter = hud_state.counters[static_cast<std::size_t>(entries[i].kind)];
            ...
            const float icon_scale =
                std::min(entries[i].icon_target_size / icon_bounds.size.x, entries[i].icon_target_size / icon_bounds.size.y);
            icon.setScale({icon_scale, icon_scale});
            icon.setPosition({icon_center_x, row_center_y});
            target.draw(icon);

            auto text = make_counter_text(font_, counter.count, 28u);
            const auto text_bounds = text.getLocalBounds();
            text.setOrigin({text_bounds.position.x + text_bounds.size.x, text_bounds.position.y + text_bounds.size.y * 0.5f});
            text.setPosition({icon_center_x - 42.0f, row_center_y - 1.0f});
            target.draw(text);

            if (counter.feedback_amount == 0 || counter.feedback_alpha <= 0.0f) continue;

            const float alpha_value = std::clamp(counter.feedback_alpha * 255.0f, 0.0f, 255.0f);
            const auto alpha = static_cast<std::uint8_t>(alpha_value);
            const std::string delta_value = std::format("{}{}", counter.feedback_amount > 0 ? "+" : "", counter.feedback_amount);
            sf::Text delta_text{font_, delta_value, 20u};
            const auto delta_bounds = delta_text.getLocalBounds();
            delta_text.setOrigin({delta_bounds.position.x + delta_bounds.size.x, delta_bounds.position.y + delta_bounds.size.y * 0.5f});
            delta_text.setFillColor(counter.feedback_amount > 0 ? with_alpha(0x4FF471_rgb, alpha) : with_alpha(0xFF5555_rgb, alpha));
            ...
            delta_text.setPosition({icon_center_x - 112.0f, row_center_y - 1.0f - counter.feedback_offset_y});
