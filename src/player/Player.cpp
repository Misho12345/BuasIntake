...
namespace game::player
{
    namespace
    {
        ...
        std::unique_ptr<sf::ConvexShape> make_capsule_drawable(const float radius,
                                                               ...
                                                               const std::size_t arc_segments = 12)
        {
            ...
            for (std::size_t i = 0; i <= arc_segments; ++i)
            {
                ...
                shape->setPoint(i, {std::cos(angle) * radius, top_center_y + std::sin(angle) * radius});
            }
            ...
            for (std::size_t i = 0; i <= arc_segments; ++i)
            {
                ...
                shape->setPoint(top_arc_count + i, {std::cos(angle) * radius, bottom_center_y + std::sin(angle) * radius});
            }
        ...
        float ground_ray_cast_callback(
            const b2ShapeId shape_id, const b2Vec2 point, const b2Vec2 normal, const float fraction, void* context)
        {
    ...
    Result<void> Player::create(const b2WorldId world_id, const vec2 spawn_position, const vec2 planet_center, const PlayerConfig& config)
    {
        destroy();
    ...
    Result<void> Player::create_capsule_shape()
    {
        ...
        const b2Capsule capsule{.center1 = {.x = 0.0f, .y = -capsule_center_offset},
    ...
    Result<void> Player::create_ground_sensor_shape()
    {
        ...
        const b2Polygon ground_sensor =
            b2MakeOffsetBox(sensor_half_width, sensor_half_height, {.x = 0.0f, .y = sensor_offset_y}, b2Rot_identity);
        ...
        if (!b2Shape_IsValid(ground_sensor_shape_))
            return fail("Failed to create player ground sensor shape");
        return {};
    }
    ...
    Result<void> Player::create_water_sensor_shape()
    {
        ...
        if (!b2Shape_IsValid(water_sensor_shape_))
            return fail("Failed to create player water sensor shape");
        return {};
    }
    ...
    void Player::refresh_grounded_state(const vec2 planet_center)
    {
        ...
        if (!valid() || !b2World_IsValid(world_))
            return;
    ...
    bool Player::sensor_detects_ground() const
    {
        ...
        const int overlap_count = b2Shape_GetSensorOverlaps(ground_sensor_shape_, overlaps.data(), static_cast<int>(overlaps.size()));
        for (int i = 0; i < overlap_count; ++i)
        {
            const auto overlap_shape = overlaps[static_cast<std::size_t>(i)];
            if (!b2Shape_IsValid(overlap_shape))
                continue;
            if (B2_ID_EQUALS(b2Shape_GetBody(overlap_shape), object_.body))
                continue;
            if (b2Shape_IsSensor(overlap_shape))
                continue;
    ...
    bool Player::sensor_detects_water() const
    {
        ...
        std::array<b2ShapeId, sensor_overlap_capacity> overlaps{};
        const int overlap_count = b2Shape_GetSensorOverlaps(water_sensor_shape_, overlaps.data(), static_cast<int>(overlaps.size()));
        for (int i = 0; i < overlap_count; ++i)
        {
            const auto overlap_shape = overlaps[static_cast<std::size_t>(i)];
            if (!b2Shape_IsValid(overlap_shape))
    ...
    std::optional<vec2> Player::raycast_ground_normal(const vec2 planet_center) const
    {
        ...
        b2World_CastRay(world_, to_b2(ray_origin), to_b2(ray_translation), filter, ground_ray_cast_callback, &ray_context);

        if (!ray_context.hit)
            return std::nullopt;

        const auto normal = normalize(ray_context.normal, up);
        if (normal.dot(up) < config_.ground_min_normal_dot)
            return std::nullopt;
        return normal;
    }

    void Player::prepare_for_physics_step(const float fixed_step,
    ...
    {
        object_.draw_sf(target);
    }

    b2BodyId Player::body() const
    {
        return object_.body;
    }
    bool Player::valid() const
    {
        return b2Body_IsValid(object_.body);
    }

    vec2 Player::world_position() const
    {
        if (!valid())
            return object_.transformable.getPosition();
        return from_b2(b2Body_GetPosition(object_.body));
    }

    vec2 Player::up_direction(const vec2 planet_center) const
    {
        return normalize(world_position() - planet_center);
    }
    ...
    {
        return (input.is_pressed(Key::D) ? 1.0f : 0.0f) - (input.is_pressed(Key::A) ? 1.0f : 0.0f);
    }
    ...
    vec2 Player::movement_direction(const vec2 up_direction) const
    {
        ...
        if (ground_alignment < config_.ground_min_normal_dot)
            return right_direction;
    ...
    void Player::apply_horizontal_movement(const float fixed_step,
                                           ...
                                           const platform::InputSystem& input)
    {
        ...
        if (move_axis != 0.0f)
        {
            ...
            const float speed_change = std::clamp(desired_tangent_speed - current_tangent_speed, -max_speed_change, max_speed_change);
    ...
    void Player::try_jump(const vec2 up_direction, const bool jump_held, const bool in_water)
    {
        ...
        if (!grounded_ || jump_cooldown_timer_ > 0.0f)
            return;
    ...
    void Player::apply_input(const float fixed_step, const vec2 planet_center, const bool in_water, const platform::InputSystem& input)
    {
        if (!valid())
            return;
    ...
    void Player::apply_gravity(const vec2 planet_center, const bool in_water) const
    {
        ...
        const vec2 gravity_force =
            up_direction(planet_center) * (-b2Body_GetMass(object_.body) * config_.gravity_acceleration * gravity_scale);
    ...
    void Player::align_to_planet(const vec2 planet_center) const
    {
        ...
        b2Body_SetAngularVelocity(object_.body, 0.0f);
        if (std::abs(shortest_angle_delta(current_angle, target_angle)) <= 1e-4f)
            return;
