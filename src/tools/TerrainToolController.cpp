...
namespace game::tools
{
    ...
    bool TerrainToolController::should_suppress_world_input_after_modal(const TerrainToolContext& context)
    {
        ...
        if (context.input->just_pressed(MouseButton::Left) ||
            context.input->just_pressed(MouseButton::Right))
        {
            require_fresh_mouse_press_after_modal_ = false;
            return false;
        }
    ...
    void TerrainToolController::handle_upgrade_menu_click(
        const TerrainToolContext& context,
        const vec2                ui_position,
        const uvec2               target_size)
    {
    ...
    void TerrainToolController::draw_ui(sf::RenderTarget& target) const
    {
        ...
        upgrade_menu_.draw(
            ...
            upgrade_model::build_menu_cards(
                terrain_tool_,
                water_tool_,
                [this](const std::size_t column, const std::size_t row) { return tool_icon_rect(column, row); }));
    }
    ...
    TerrainTool& TerrainToolController::active_tool()
    {
        ...
        {
            case HotbarSlot::Digging: return terrain_tool_;
            case HotbarSlot::Water: return water_tool_;
            case HotbarSlot::Seeds: return seed_tool_;
        }
    ...
    std::array<
        ...
        TerrainToolController::hotbar_slot_count
    > TerrainToolController::build_hud_slots() const
    {
        return {
    ...
    TerrainToolHudSlotData TerrainToolController::build_digging_slot_data() const
    {
        const float fill_ratio = terrain_tool_.capacity() == 0u
                                     ? 0.0f
                                     : static_cast<float>(terrain_tool_.stored_ground()) /
                                     static_cast<float>(terrain_tool_.capacity());
    ...
    {
        const float fill_ratio = water_tool_.current_capacity() == 0u
                                     ? 0.0f
        ...
        const std::optional<float> overlay_ratio =
                water_tool_.is_placement_mode() &&
                water_tool_.current_capacity() > 0u
                    ? std::optional
                    {
