...
namespace game::tools
{
    namespace
    {
        void draw_panel(sf::RenderTarget& target,
        ...
        void draw_slot_meter(sf::RenderTarget& target,
                             const sf::Vector2f position,
                             const sf::Vector2f size,
                             const float ratio,
                             ...
                             const sf::Color overlay_color = sf::Color::Transparent)
        {
            ...
            const sf::Vector2f inner_size{std::max(size.x - 4.0f, 0.0f), std::max(size.y - 4.0f, 0.0f)};
    ...
    void TerrainToolHudRenderer::draw(sf::RenderTarget& target, const std::span<const TerrainToolHudSlotData> slots)
    {
        ...
        const float hotbar_width =
            panel_padding_x * 2.0f + slot_size * static_cast<float>(slots.size()) + slot_gap * static_cast<float>(slots.size() - 1u);
        const float hotbar_height = panel_padding_y * 2.0f + slot_size;
        const sf::Vector2f hotbar_position{width - hotbar_width - 22.0f, height - hotbar_height - 22.0f};

        draw_panel(target, hotbar_position, {hotbar_width, hotbar_height}, 0x0D141CE4_rgba, 0x3F5261EC_rgba, 2.0f);

        for (std::size_t i = 0; i < slots.size(); ++i)
        {
            const auto& slot = slots[i];
            const sf::Vector2f slot_position{hotbar_position.x + panel_padding_x + static_cast<float>(i) * (slot_size + slot_gap),
