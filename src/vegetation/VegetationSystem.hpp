...
namespace game::vegetation
{
    ...
    public:
        ...
        Result<void> plant_seed(const terrain::PlanetTerrain& terrain, resources::ResourceSystem& resources, vec2 world_position);
        bool update(float dt, const terrain::PlanetTerrain& terrain);
        void clear_plant_at(std::size_t sample_index);
        void refresh_surface_anchors(const terrain::PlanetTerrain& terrain, const std::unordered_set<std::uint64_t>& affected_keys);
        void validate() const;

        std::span<const Plant> plant_samples() const noexcept
        {
            return plant_samples_;
        }
    ...
    private:
        ...
        std::uint32_t nearby_cover_count(const terrain::PlanetTerrain& terrain, ivec2 coord, float radius_samples, bool woody_cover) const;
