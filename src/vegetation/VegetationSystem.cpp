...
namespace game::vegetation
{
    namespace
    {
        ...
        bool is_solid(const terrain::PlanetTerrain::FieldSample& sample)
        {
            return sample.terrain >= 0.0f;
        }

        bool has_water(const terrain::PlanetTerrain::FieldSample& sample)
        {
    ...
    std::optional<ivec2> VegetationSystem::find_plantable_seed_coord(const terrain::PlanetTerrain& terrain, const vec2 world_position) const
    {
        ...
        for (int y = center.y - search_radius; y <= center.y + search_radius; ++y)
        {
            for (int x = center.x - search_radius; x <= center.x + search_radius; ++x)
            {
                ...
                const auto field_size = terrain.global_field_size();
                const auto index =
                    static_cast<std::size_t>(coord.y) * static_cast<std::size_t>(field_size.x) + static_cast<std::size_t>(coord.x);
                ...
                else
                {
                    ...
                    if (nearby_cover_count(terrain, coord, family == PlantFamily::Tree ? 16.0f : 9.0f, true) >=
                        (family == PlantFamily::Tree ? 1u : 3u))
                        continue;
                }
	    ...
	    const int                     min_spacing_samples) const
    {
        ...
        for (const auto index : active_plant_indices())
        {
            ...
            if (dx * dx + dy * dy < min_spacing_samples * min_spacing_samples)
                return false;
        }
    ...
    PlantFamily VegetationSystem::choose_plant_family(const terrain::PlanetTerrain& terrain, const ivec2 coord) const
    {
        ...
        const std::uint32_t active_count = static_cast<std::uint32_t>(std::max<std::size_t>(active_plant_indices().size(), 1u));
        const float tree_ratio = static_cast<float>(mature_tree_count()) / static_cast<float>(active_count);

        if (roll > 0.86f && tree_ratio < 0.36f && can_place_woody_near(terrain, coord, 12))
            return PlantFamily::Tree;
        if (roll > 0.76f)
            return PlantFamily::Bush;
        if (roll > 0.75f)
            return PlantFamily::Flowers;
        return PlantFamily::Grass;
    }

    std::uint8_t VegetationSystem::choose_plant_variant(const terrain::PlanetTerrain& terrain,
                                                        const ivec2 coord,
                                                        const PlantFamily family) const
    {
        const float random_value = hash01(static_cast<float>(coord.x), static_cast<float>(coord.y), terrain.seed() + 911u);
        const std::uint8_t base_variant = static_cast<std::uint8_t>(std::clamp(static_cast<int>(random_value * 16.0f), 0, 15));
        if (family != PlantFamily::Tree)
            return base_variant;
    ...
    bool VegetationSystem::spread_plants(const terrain::PlanetTerrain& terrain)
    {
        ...
        for (const auto index : active_plant_indices_)
        {
            ...
            const ivec2 origin{ static_cast<int>(index % field_size.x), static_cast<int>(index / field_size.x) };
            int radius = 5;
            if (plant.family == PlantFamily::Tree) radius = 15;
            else if (plant.family == PlantFamily::Bush) radius = 8;
            const int start_offset =
                static_cast<int>(hash01(static_cast<float>(origin.x), static_cast<float>(origin.y), terrain.seed() + 7103u) * 32.0f);

            static constexpr std::array offsets{
                ivec2{ 1, 0 },   ivec2{ 2, 0 },  ivec2{ 3, 1 },   ivec2{ 2, 2 },   ivec2{ 1, 3 },   ivec2{ 0, 2 },  ivec2{ -1, 3 }, ivec2{ -2, 2 },
                ivec2{ -3, 1 },  ivec2{ -2, 0 }, ivec2{ -3, -1 }, ivec2{ -2, -2 }, ivec2{ -1, -3 }, ivec2{ 0, -2 }, ivec2{ 1, -3 }, ivec2{ 2, -2 },
                ivec2{ 3, -1 },  ivec2{ 4, 1 },  ivec2{ 4, -1 },  ivec2{ -4, 1 },  ivec2{ -4, -1 }, ivec2{ 1, 4 },  ivec2{ -1, 4 }, ivec2{ 1, -4 },
                ivec2{ -1, -4 }, ivec2{ 5, 0 },  ivec2{ -5, 0 },  ivec2{ 0, 5 },   ivec2{ 0, -5 },  ivec2{ 6, 2 },  ivec2{ -6, 2 }, ivec2{ 2, -6 }
            };
            ...
            for (std::size_t attempt = 0; attempt < offsets.size(); ++attempt)
            {
                ...
                if (!terrain.is_valid_global_sample(coord))
                    continue;
                if (!terrain.is_surface_suitable_for_plant(coord))
                    continue;

                const auto target_index =
                    static_cast<std::size_t>(coord.y) * static_cast<std::size_t>(field_size.x) + static_cast<std::size_t>(coord.x);
                const auto& sample = terrain.global_sample(coord);
                if (!is_solid(sample) || has_water(sample) || sample.wetness < seed_plantable_wetness_threshold)
                    continue;
                if (plant_samples_[target_index].stage != PlantStage::Empty)
                    continue;
                if (terrain.has_resource_at(coord))
                    continue;

                const auto family = choose_plant_family(terrain, coord);
                if (is_low_cover_family(family))
                {
                    if (nearby_cover_count(terrain, coord, 4.8f, false) >= 4u)
                        continue;
                }
                else
                {
                    const int spacing = family == PlantFamily::Tree ? 12 : 10;
                    if (!can_place_woody_near(terrain, coord, spacing))
                        continue;
                    if (nearby_cover_count(terrain, coord, family == PlantFamily::Tree ? 11.0f : 8.5f, true) >= 4u)
                        continue;
                }
        ...
        for (const auto& spawned : spawned_plants)
        {
            if (spawned.index >= plant_samples_.size())
                continue;
            if (plant_samples_[spawned.index].stage != PlantStage::Empty)
                continue;
    ...
    {
        std::erase_if(active_plant_indices_,
