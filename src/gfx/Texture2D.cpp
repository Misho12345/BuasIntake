...
namespace game::gfx
{
    Result<void> Texture2D::create(const uvec2 size, const TextureFormat format)
    {
        ...
        glTextureStorage2D(handle_, 1, to_gl_format(format_), static_cast<GLsizei>(size_.x), static_cast<GLsizei>(size_.y));
    ...
    GLuint Texture2D::native_handle() const
    {
        return handle_;
    }

    uvec2 Texture2D::size() const
    {
        return size_;
    }
